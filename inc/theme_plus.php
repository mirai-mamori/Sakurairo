...
function the_video_headPattern(bool $isHls = false)
{
    ...
    if (is_single()) {
        while (have_posts()) {
            ...
            //$ava = iro_opt('personal_avatar', '') ? iro_opt('personal_avatar', '') : get_avatar_url(get_the_author_meta('user_email'));
            $edit_this_post_link = get_edit_html();
            $btn_playControl = '<button id="cv-pc" class="coverVideo-btn" onclick="coverVideo()"><i class="fa-solid fa-pause" aria-hidden="true"></i></button>';
/*             $btn_volumeControl = '<button id="cv-vc" class="coverVideo-btn" onclick="coverVideoMute()"><i class="fa-solid fa-volume-off" aria-hidden="true"></i></button>';
...
function siren_private_message_hook($comment_content , $comment){
    ...
    if ( $is_private ) return '<i class="fa-solid fa-lock" aria-hidden="true"></i> '.__("The comment is private","sakurairo")/*该评论为私密评论*/;
