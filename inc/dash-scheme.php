...
function _get($str){
    ...
    // 防御数组型参数（如 ?color_1[]=x），避免 strip_tags()/stripslashes() 在 PHP 8 抛出 TypeError
    if (empty($_GET[$str]) || is_array($_GET[$str])) {
        return null;
    }
    $val = strip_tags(stripslashes($_GET[$str]));
    return $val;
}
...
if(_get('rules')==NULL) {
	...
	$rules = preg_replace('/@import/i', '/* blocked */', $rules);
